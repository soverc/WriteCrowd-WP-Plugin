<?php 
	$oneighty = wp_oneighty_article_info(get_the_ID()); 
?>
<style type="text/css">
		...
		setTimeout(function(){
			jQuery.ajax({
				...
				data: {
					...
					article_id: '<?php _e($oneighty_post_wp_id) ?>'
				},
				dataType: 'html',
				type: 'post',
				async: false,
...
<div id="oneighty_login_div">
		<div>
			<label>Login Method : </label>
				<br />
					<input type="radio" name="mp_user_login_method" value="wordpress" />
						<img src="<?php echo($mp_defs['images_path']) ?>/wordpress-logo.png" width="200" height="50" />
				<br />
					<input type="radio" name="mp_user_login_method" value="mediaplace" />
						<img src="<?php echo($mp_defs['images_path']) ?>/180create-logo.png" width="200" height="50" />
		</div>
	...
	<form action="" method="post" id="oneighty_comment_form">
		...
		<!-- Comment Body -->
	   	...
	   	<p>
	       	...
	       	<input name="oneighty_comment_post_id" id="oneighty_comment_post_id" value="<?php _e($oneighty->mediaplace_id)?>" type="hidden">
	   	</p>
	</form>
</div>
