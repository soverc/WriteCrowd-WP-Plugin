...
class Wc_Actions {
	...
	public function doRemoveArticle($aid) {
		...
		// 180Create article
		if ($this->checkIfOneightyArticle($aid) === true) {
			...
			if ($this->checkIfArticleBelongsToOneightyAccount() === true) {
				...
				$this->feedJson(array(
					...
					'article_id' => $this->getArticle()->aSyndicationData[str_replace('{nameSpace}', $this->getNameSpace(), Wc_Config::Get('wordPress', 'postMetaKeyData'))]->id
				));
			...
			$this->setError(str_replace(array(
				...
				$aid, Wc_Config::Get('variables', 'appName')
			), Wc_Config::Get('errorMessages', 'postNotFromSyndicate')));
	...
	public function checkIfOneightyArticle($iWordPressId) {
		...
		if (!empty($iWordPressId) && (intval(get_post_meta($iWordPressId, str_replace('{nameSpace}', $this->getNamespace(), Wc_Config::Get('wordPress', 'postMetaKeySyndicated')), true)) == true)) {
	...
	public function renderComments($sTemplate) {
		...
		if ($this->checkIfOneightyArticle(get_the_ID()) === true) {
			...
			// If so, load our template
			// for logged in users
			$sTemplate = "{$this->getPluginPath()}/".Wc_Config::Get('folders', 'templates')."/{$this->getNameSpace()}_comments.php";
		}
		...
		if ($this->checkForOneightyAccount()) {
			...
			if (!empty($_POST) && isset($_POST[$this->getNameSpace()])) {
				...
				if ($this->checkIfOneightyArticle($aPostData['iWordPressId']) === false) {
					...
					// article prefix
					if (Wc_Config::Get('variables', 'enableArticleTitlePrefix')) {

						// If so, append the prefix to the
						// beginning of the article title
						$oArticle->sTitle = (string) Wc_Config::Get('variables', 'articleTitlePrefix').$oPost->post_title;
					} else {
					...
					if (Wc_Config::Get('variables', 'enableSubCategories')) {
					...
					if (Wc_Config::Get('variables', 'enableGroups')) {
					...
					if (Wc_Config::Get('variables', 'enablePrivacySettings')) {
					...
					if (Wc_Config::Get('variables', 'enableAllowFree')) {
					...
					if (Wc_Config::Get('variables', 'enableCostSelect')) {
					...
					// Set system error
					$this->setError(Wc_Config::Get('errorMessages', 'articleAlreadyExists'));
				}
			}
	...
	public function renderLogout() {
		...
		if ($this->doLogout()) {
			...
			// Set the system error
			$this->setError(Wc_Config::Get('errorMessages', 'cannotLogout'));
	...
	public function renderPostContent($sContent) {
		...
		if ($this->checkIfOneightyArticle(get_the_ID()) === true) {
			...
			$sContent .= '<p><center><a href="'.Wc_Config::Get('urls', 'baseUrl').'">';
			$sContent .= '<img src="'.Wc_Config::Get('urls', 'baseUrl').'/images/syndicated.png" alt="Syndicated at '.Wc_Config::Get('variables', 'appName').'">';
			$sContent .= '</a></center></p>';
			$sContent .= '<script type="text/javascript">';
			$sContent .= 'jQuery(\'.entry-meta\').html(\'<p><em>By </em><strong><a href="'.Wc_Config::Get('urls', 'baseUrl').'/user/'.$this->getArticle()->aSyndicationData[$sMetaProperty]->author_name.'">'.$this->getArticle()->aSyndicationData[$sMetaProperty]->author_name.'</a></strong><em>, Originally Published at </em><a href="'.$this->getArticle()->aSyndicationData[$sMetaProperty]->from_url.'">'.$this->getArticle()->aSyndicationData[$sMetaProperty]->from_url.'</a></p>\');';
			$sContent .= '</script>';
		}
				...
				if (isset($aPostData['rdoSyndicate']) && ($aPostData['rdoSyndicate'] == 1) && ($this->checkIfOneightyArticle($iWordPressId) === false)) {
					...
					if (!empty($_POST['content'])) {
						...
						if (Wc_Config::Get('variables', 'enableArticleTitlePrefix')) {
							...
							// If so, append the prefix to the
							// beginning of the article title
							$oArticle->sTitle = (string) Wc_Config::Get('variables', 'articleTitlePrefix').$_POST['post_title'];
						} else {
						...
						if ($this->checkIfOneightyArticle($this->getArticle()->iWordPressId) === false) {
							...
							if ($this->doSyndicateToRpc()) {
								...
								// Set the system error
								$this->setError(Wc_Config::Get('errorMessages', 'unableToSyndicateFromWordPress'));
						...
						// Set the system error
						$this->setError(Wc_Config::Get('errorMessages', 'noPostBody'));
					}
				}
			}
		}
	...
	public function generateCategoriesDropdown($bSecondCategory = false) {
		...
		if ($this->checkForData('getCategories')) {
			...
			// set system errors
			$this->setError(Wc_Config::Get('errorMessages', 'noCategoriesLoaded'));
	...
	public function generateHtmlButton($sName, $sIdentifier, $sValue, $aAttributes = array()) {
		...
		// Start the HTML
		$sHtml = (string) "<button name=\"{$this->getNameSpace()}[{$sName}]\" id=\"{$this->getNameSpace()}-{$sIdentifier}\" ";
	...
	public function generateHtmlDropdown($sName, $sIdentifier,array $aDataProvider,array $aAttributes = array()) {
		...
		// Start the HTML
		$sHtml = (string) "<select name=\"{$this->getNamespace()}[{$sName}]\" id=\"{$this->getNamespace()}-{$sIdentifier}\" ";
		...
		if (isset($aDataProvider[0]) && is_object($aDataProvider[0])) {
			...
			foreach ($aDataProvider as $oDataEntry) {
				...
				// Option placeholder
				$sOption = (string) "<option value=\"{$oDataEntry->sValue}\" ";
				...
				$sOption .= (string) ">{$oDataEntry->sLabel}</option>";
	...
	public function generateHtmlFormField($sType, $sName, $sIdentifier,array $aAttributes = array()) {
		...
		// Start the tag
		$sHtml = (string) "<input type=\"{$sType}\" name=\"{$this->getNamespace()}[{$sName}]\" id=\"{$this->getNamespace()}-{$sIdentifier}\" ";

		// Check for other specified attributed
		if (!empty($aAttributes)) {

			// If there are attribute, loop
			// through them and append them
			// to the HTML string
			foreach ($aAttributes as $sKey => $sVal) {

				// Append the attribute to the string
				$sHtml .= (string) "{$sKey} =\"{$sVal}\" ";
			}
		}
	...
	public function generateSubcategoriesDropdown($iCategoryId, $bSecondCategory = false) {
		...
		if ($this->checkForData('getSubcategories')) {
			...
			// There are no categories,
			// set system errors
			$this->setError(Wc_Config::Get('errorMessages', 'noSubcategoriesLoaded'));
		...
		if ($this->checkForData('getPostData')) {
			...
			if (isset($aPostData['sRoute']) && !is_null($aPostData['sRoute'])) {
				...
				if (method_exists($this, "__{$this->doSanitize($aPostData['sRoute'])}")) {
					...
					// set system error
					$this->setError(str_replace('{ajaxMethod}', $aPostData['sRoute'], Wc_Config::Get('errorMessages', 'ajaxMethodNoExist')));
				...
				// set the system error
				$this->setError(Wc_Config::Get('errorMessages', 'noAjaxRoute'));
	...
	private function __ajaxRemoveArticle() {
		...
		if ($this->doRemoveArticle($aPostData['iWordPressId']) === true) {

			// The article was removed,
			// return successful ajax
			$sReturnJson = json_encode(array(
				'bSuccess' => true,
				'sMessage' => Wc_Config::Get('successMessages', 'removedFromWordPress')
			));
	...
	public function __ajaxSyndicateArticle() {
		...
		if ($this->doSyndicateToWordPress()) {
			...
			$sReturnJson = json_encode(array(
				'bSuccess' => true,
				'sMessage' => Wc_Config::Get('successMessages', 'syndicatedToWordPress')
			));
			...
			// set the system error
			$this->setError(Wc_Config::Get('errorMessages', 'noSyndicateToWordPress'));
	...
	public function feedJson(array $aData, $bArray = false) {
		...
		if (!isset($aData['_method']) || is_null($aData['_method'])) {
			...
			// set the system error and
			// return false
			$this->setError(Wc_Config::Get('errorMessages', 'noRpcMethod'));
		...
		// Send the request
		$mResponse = file_get_contents(Wc_Config::Get('feeds', 'jsonRpc'), false, $rContext);
