...
<rsd version="1.0" xmlns="http://archipelago.phrasewise.com/rsd">
  <service>
    ...
    <apis>
      <api name="WordPress" blogID="1" preferred="false" apiLink="<?php echo site_url('xmlrpc.php', 'rpc') ?>" />
	  ...
	  <api name="oneighty" blogID="" preferred="true" apilink="<?php echo WP_CONTENT.'/plugins/oneighty/xml/rpc.php' ?>" />
    </apis>
  </service>
</rsd>
<?php
...
class wp_xmlrpc_server extends IXR_Server {
	...
	function wp_xmlrpc_server() {
		$this->methods = array(
			'oneighty.push'                  => 'this:oneighty_push',
			'oneighty.pull'                  => 'this:oneighty_pull',
			'oneighty.update'                => 'this:oneighty_update',
			'oneighty.auth_external_credits' => 'this:oneighty_external_credits'
		);
	...
	/*
	 * Our 180Create Functions
	 */
	function oneighty_push($args) {
		global $wpdb;
		...
		if ( ! $user = $this->login($uname, $passwd)) {
			return($this->error);
		}
		...
		$new_post_id   = wp_insert_post($post);
			$query = 
				"INSERT INTO {$wpdb->prefix}oneighty_syndicated_posts (
					post_id, 
					oneighty_id, 
					oneighty_category, 
					oneighty_secondcategory, 
					oneighty_cost, 
					oneighty_author_name, 
					oneighty_created,
					oneighty_syndicated
				) VALUES (
					{$new_post_id},
	...
	function oneighty_pull($args) {
	
	}
	
	function oneighty_update($args) {
	
	}
	
	function oneighty_external_credits($args) {
		global $wpdb;
		
		$token = strip_tags($args[0]);
		$ip    = strip_tags($args[1]);
		
		if (null != $token) {
			if (count($wpdb->get_results("SELECT * FROM {$wpdb->prefix}oneighty_external_credit_store WHERE ip = '{$ip}'"))) {
				$sql = "UPDATE {$wpdb->prefix}oneighty_external_credit_store SET token = '{$token}' WHERE ip = '{$ip}'";
			} else {
				$sql = "INSERT INTO {$wpdb->prefix}oneighty_external_credit_store (ip, token, created) VALUES ('{$ip}', '{$token}', '".date('Y-m-d H:i:s')."')";
			}
